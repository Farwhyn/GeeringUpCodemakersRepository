  ...
  switch (level) {
    ...
    for( var row = 0; row < CANVAS_HEIGHT/GRID_HEIGHT; row++){
  		for(var col = 0; col < CANVAS_WIDTH/GRID_WIDTH; col++){
  			...
  			var newGrid = new Grid(col*GRID_WIDTH,row*GRID_HEIGHT , mazeContent1[row][col],beanType);
    ...
    for( var row = 0; row < CANVAS_HEIGHT/GRID_HEIGHT; row++){
  		for(var col = 0; col < CANVAS_WIDTH/GRID_WIDTH; col++){
  			var beanType = NORMAL_BEAN;
  			var newGrid = new Grid(col*GRID_WIDTH,row*GRID_HEIGHT , mazeContent2[row][col],beanType);
  	...
  	//overwrite beans that shouldn't ecist
    noBean = [pacmanStartLoc, [1,1], [1,2], [1,3], [1,5], [1,6], [1,7], [1,10], [1,11], [1,12], [1,14], [1,15], [1,16],
                              [4,0], [4,1], [4,2], [4,3], [4,14], [4,15], [4,16],
                              [5,0], [5,1], [5,2], [5,3], [5,14], [5,15], [5,16],
                              [6,6], [6,7], [6,8], [6,9], [6,10], [6,11],
                              [7,6], [7,11], [7,7], [7,8], [7,9], [7,10],
                              [8,6], [8,7], [8,8], [8,9], [8,10], [8,11],
                              [9,1], [9,2], [9,3], [9,4], [9,6], [9,7], [9,8], [9,9], [9,10], [9,11], [9,13], [9,14], [9,15], [9,16],
                              [13,0], [13,1], [13,2], [13,3], [13,14], [13,15], [13,16],
                              [14,0], [14,1], [14,2], [14,3], [14,14], [14,15], [14,16]];
  	for(var i=0; i<noBean.length; i++){
  		var x = noBean[i][0];
  		var y = noBean[i][1];
  		maze[x][y].beanType = undefined;
...
function initFields () {
  ...
  switch (level) {
    ...
    case 2:
    for (var i=7; i<11; i++){
  		ghostHouse[ghostHouseIndex]=[8, i];
  		ghostHouseIndex++;
  	}
    break;
